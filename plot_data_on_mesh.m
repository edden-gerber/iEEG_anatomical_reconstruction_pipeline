...
%   labeled as 1,2,3...). 
% PLOT_DATA_ON_MESH(...,h) where h is a Matlab handle to a patch (mesh)
%   object, will apply the plotting to the given object. 
% PLOT_DATA_ON_MESH(...,'new_color_layer',B) where B is true or false, will
%   deteermine whether the plotted data will replace the current object 
%   coloring or painted over it. Default is true. 
% 
%
...
elseif size(coordinates,2) == 3
    coordinates = coordinates(~ignore_coord,:);
else
...
while arg <= narg
    if ischar(varargin{arg})
        switch lower(varargin{arg})
            ...
            case 'shownumbers'
                show_numbers = true;
                arg = arg + 1;
            case 'new_color_layer'
                if islogical(varargin{arg+1}) | (isscalar(varargin{arg+1}) & (varargin{arg+1}==0 | varargin{arg+1}==1))
                    add_new_layer = logical(varargin{arg+1});
                    arg = arg + 2;
                else
                    error('"new_color_layer" argument should be followed by true or false');
                end
            otherwise
    ...
    elseif isscalar(varargin{arg}) && ishandle(varargin{arg})
        mesh_handle = varargin{arg};
        arg = arg + 1;
...
switch method
    ...
    case 'surface'
        ...
        if patch_size > 0
            ...
            for e = 1:size(coordinates,1)
                ...
                elseif strcmp(surface_overlap_method,'mean') % overlapping values are summed and then averaged (after this loop)
                    ...
                    vertex_color_values = sum(v .* [max(abs(v))==abs(v(1,:)) ; max(abs(v))~=abs(v(1,:))]);
                end
            end
        ...
        paint_mesh(vertex_color_values', transparency_map, add_new_layer, mesh_handle);
        colormap(color_map);
