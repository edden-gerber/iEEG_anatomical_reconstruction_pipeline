...
% "subject ID" is fsaverage
subj_id = 'fsaverage';

% Create template home directory 
recon_folder_home = fullfile(initial_folder,'fsaverage');
if ~exist(recon_folder_home,'dir')
    disp(['Making new folder: ' recon_folder_home]);
    mkdir(recon_folder_home);
end
disp(['Recon home folder is ' recon_folder_home]);

%% Look at the fsaverage brain using freeview
...
% This creates a SUMA folder in the FreeSurfer subject folder. You may need
% to change permissions to the fsaverage folder in FreeSurfer's subjects
% folder. 
